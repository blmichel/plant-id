...
#        self.save_hyperparameters()
        ...
        self.lr = self.args.get("lr", config['LR'])
        self.wd = self.args.get("wd", config['WEIGHT_DECAY'])

        self.loss = self.args.get("loss", config['LOSS'])
        if self.loss == "arcface":
            self.loss_fn = losses.ArcFaceLoss(num_classes=NUM_PLANTS, embedding_size=NUM_PLANTS)
        else:
            self.loss_fn = getattr(torch.nn.functional, self.loss)

        # experiment with OneCycleLR
        self.one_cycle_max_lr = self.args.get("one_cycle_max_lr", None)
        #self.one_cycle_max_lr = self.args.get("one_cycle_max_lr", True)
        self.one_cycle_total_steps = self.args.get("one_cycle_total_steps", config['ONE_CYCLE_TOTAL_STEPS'])
    ...
    def configure_optimizers(self):
        ...
        if self.one_cycle_max_lr is None:
            ...
            assert not config['REDUCE_LR_ON_PLATEAU'], "Error: 1cycle scheduler incompatible with ReduceLROnPlateau"
